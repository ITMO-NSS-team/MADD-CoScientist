...
def bot(history: list[list]) -> list[list]:
    ...
    try:
        ...
        onlyfiles = [
            f
            for f in listdir("multi_agents_system/vizualization")
            if isfile(join("multi_agents_system/vizualization", f))
        ]
    except:
        answer = chain.run(history[-1][0])
        onlyfiles = [
    ...
    try:
        if onlyfiles != []:
            ...
            for file in sorted_files[::-1]:
                image = gr.Image("multi_agents_system/vizualization/" + file)
