...
with open("multi_agent_system/six_case_multi_agents_proto/config.yaml", "r") as file:
...
def bot(history: list[list]) -> list[list]:
    ...
    try:
        ...
        onlyfiles = [
            f
            for f in listdir(
                "multi_agent_system/six_case_multi_agents_proto/vizualization"
            )
            if isfile(
                join("multi_agent_system/six_case_multi_agents_proto/vizualization", f)
            )
        ]
    except:
        answer = chain.run(history[-1][0])
    ...
    # if answer without molecules
        ...
        if onlyfiles != []:
            for file in onlyfiles:
                image = gr.Image(
                    "multi_agent_system/six_case_multi_agents_protovizualization/"
                    + file
                )
