...
def ml_dl_agent(state, config: dict):
    user_query = state["input"] if state.get('language', 'English') == 'English' else state['translation']
    ...
    agent = create_react_agent(llm, [predict_prop_by_smiles, train_ml_with_data, get_state_from_server], state_modifier=prompt)
