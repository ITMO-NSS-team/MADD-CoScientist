...
from prompts.parsers import chat_parser, planner_parser, supervisor_parser, replanner_parser, translator_parser
from tools import tools_rendered, web_tools_rendered, nano_tools_rendered, chem_tools_rendered


worker_prompt = "You are a helpful assistant. You can use provided tools. If there is no appropriate tool, or you can't use one, answer yourself"
...
Your plan is: Convert the IUPAC name of the deuterio 3-[deuterio(1,1,3,3,4,4,4-heptadeuteriobutyl)amino]-5-(dideuteriosulfamoyl)-4-phenoxybenzoate to SMILES format using the name2smiles function with the given IUPAC name as input.
ONLY return JSON in this exact format: {{"steps": ["Step 1", "Step 2", "Step 3"]}}. Don't add any introduction""" + f'System has these tools {tools_rendered}',
        ),
...
replanner_prompt = ChatPromptTemplate.from_template(
 """For the given objective, come up with a simple step by step plan how to answer the question. You can't answer yourself \
This plan should involve individual tasks, that if executed correctly by other workers will yield the correct answer. Do not add any superfluous steps. \
You can't refer to results of previous steps. Instead you must directly insert such results in your plan. If you see step number more than 15, you should generate final response \
The result of the final step should be the final answer. Make sure that each step has all the information needed - do not skip steps. Do no more than 3-5 steps.
If you see automl related task in past_steps and it has answer, the job is done, don't use automl anymore.
...
Update your plan accordingly. If no more steps are needed and you can return to the user, then respond with final response, which answers the objective. Make sure the answer is clear. Otherwise, fill out the plan. Only add steps to the plan that still NEED to be done. Do not return previously done steps as part of the plan.
"""
...
"""Translate given input to given language. Don't 'translation to ...', just write the translation itself.
...
"""Your task is to formulate final answer based on system_response using intermediate_thoughts to make sure user gets full answer to their query.
Your response must be direct answer to user query, don't write too much text. 
You can use phrases like: it have been done, etc. Instead you must directly tell what have been done, extract all important results
