...
def predict_prop_by_smiles(smiles_list : List[str],
                   case : str = "test_api",
                   timeout : int = 20) -> Tuple[requests.Response, dict]:
    """
...
def train_ml_with_data(
    ...
    target_column=['Docking score'], #All propreties from dataframe you want to calculate in the end,
    ...
    #Get state from server
    #resp = requests.post(url,json.dumps(params))
    p = Process(target=requests.post, args=["http://10.64.4.243:81/train_ml", json.dumps(params)])
    p.start()
...
ml_dl_tools_rendered = render_text_description([get_state_from_server, train_ml_with_data, predict_prop_by_smiles])

if __name__ == "__main__":
    st = train_ml_with_data('Docking555', '/Users/alina/Desktop/ИТМО/ChemCoScientist/ChemCoScientist/dataset_handler/chembl/docking.csv')
